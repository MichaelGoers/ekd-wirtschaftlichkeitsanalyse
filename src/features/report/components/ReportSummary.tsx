  ...
  return (
    <div
      className={`flex items-baseline justify-between gap-6 py-0.5 ${
        muted ? "text-ekd-text-secondary" : "text-ekd-text"
      }`}
    >
      <span>{label}</span>
...
function SectionTitle({ children }: { children: string }) {
  return (
    <h2 className="text-xs font-semibold uppercase tracking-[0.22em] text-ekd-primary">
      {children}
    </h2>
  );
}
...
export default function ReportSummary({
  ...
  return (
    <aside className="space-y-4">
      <section className="border-b border-ekd-primary-light pb-4 text-center">
        <SectionTitle>Ihr wirtschaftlicher Vorteil</SectionTitle>
        <p className="mt-2 text-3xl font-semibold tracking-tight text-ekd-success">
          {formatCurrency(result.savingsTwentyYears)}
        </p>
        <p className="mx-auto mt-1.5 max-w-sm text-sm leading-4 text-ekd-text-secondary">
      ...
      <section>
        ...
        <div className="mt-1.5 flex items-baseline justify-between gap-6 border-t-2 border-ekd-text pt-1.5">
          <span className="font-bold text-ekd-text">
            Gesamtinvestition
          </span>
          <span className="text-right text-xl font-bold tabular-nums text-ekd-text">
      ...
      <section>
        ...
        <div className="mt-1.5 space-y-1 border-t-2 border-ekd-text pt-1.5">
          <div className="flex items-baseline justify-between gap-6 text-base font-bold leading-normal text-ekd-text">
            <span>
          ...
          <div className="flex items-baseline justify-between gap-6 text-base font-bold leading-normal text-ekd-text">
            ...
            <span className="text-right tabular-nums text-ekd-danger">
